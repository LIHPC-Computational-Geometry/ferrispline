...
from nurbs_math.geometry.bezier import (
    bernstein,
    rationalBasisBezierFunction,
    evalBezierCurve,
    computeKnotInsertionMatrix,
    bezierCurves
)
...
def test_evalBezierCurve_endpoints():
    ...
    curve = evalBezierCurve(control_points, weights, degree, sample=10)
    
    np.testing.assert_allclose(curve[0], control_points[0])
    np.testing.assert_allclose(curve[-1], control_points[-1])

def test_computeKnotInsertionMatrix_dimension():
    """Test that the extraction matrix has the correct (degree+1) x (degree+1) shape."""
    knots = [0.0, 0.0, 0.0, 1.0, 2.0, 3.0, 3.0, 3.0]
    degree = 2
    segment_index = 3
    
    matrix = computeKnotInsertionMatrix(knots, degree, segment_index)
    assert matrix.shape == (degree + 1, degree + 1)

def test_computeKnotInsertionMatrix_invalid_segment_index():
    """Test that a ValueError is raised if the segment index is out of bounds."""
    knots = [0.0, 0.0, 1.0, 1.0]
    degree = 1
    
    with pytest.raises(ValueError) as exc_info:
        computeKnotInsertionMatrix(knots, degree, segment_index=-1)
    error_msg = str(exc_info.value)
    assert "segment_index" in error_msg and "is out of bounds for knots of length" in error_msg

def test_bezierCurves_zero_weight_division():
    ...
    with pytest.raises(ValueError, match="Zero weight encountered") as exc_info:
        bezierCurves(knots, control_points, weights, degree)
