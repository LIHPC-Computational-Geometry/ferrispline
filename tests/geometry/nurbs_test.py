...
def test_eval_nurbs_curve_straight_line():
    ...
    np.testing.assert_allclose(
        curve[0],
        control_points[0],
        err_msg="Curve should start at the first control point",
    )
    np.testing.assert_allclose(
        curve[-1],
        control_points[-1],
        err_msg="Curve should end at the last control point",
    )
...
def test_eval_nurbs_curve_weight_invariance():
    ...
    curve_scaled = eval_nurbs_curve(
        knots, control_points, weights_scaled, degree, sample
    )

    np.testing.assert_allclose(
        curve_base,
        curve_scaled,
        err_msg="Uniformly scaled weights should produce the exact same curve",
    )
