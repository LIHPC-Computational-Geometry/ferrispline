...
def load_nurbs_from_vtk(
    ...
    if degree < 1:
        raise ValueError(
            "Error: the number of control point and knot are invalid: knots number = control point number + degree + 1"
        )
...
def build_knot_vector(knot_definitions: list[tuple[float, int]]) -> list:
    ...
    for value, multiplicity in knot_definitions:
        if multiplicity <= 0:
            raise ValueError(
                f"The knot multiplicity {value} must be > 0. Actually multiplicity = {multiplicity}"
            )
